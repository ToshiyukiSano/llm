...
if [[ -z ${input_tokenizer_file} ]] || [[ -z ${output_model_dir} ]] || [[ -z ${json_dir} ]] || [[ -z ${config_yaml} ]]; then
    echo "Error: Missing required arguments."
    echo "Usage: ${0} --input_tokenizer_file <input_tokenizer_file> --output_model_dir <output_model_dir> --json_dir <input_data_dir> --config_yaml <config_yaml_file>"
    exit 1
fi
...
model_size=$(yq -r '.model_size' ${config_yaml})
num_layers=$(yq -r '.num_layers' ${config_yaml})
hidden_size=$(yq -r '.hidden_size' ${config_yaml})
num_attn_heads=$(yq -r '.num_attn_heads' ${config_yaml})
global_batch_size=$(yq -r '.global_batch_size' ${config_yaml})
lr=$(yq -r '.lr' ${config_yaml})
min_lr=$(yq -r '.min_lr' ${config_yaml})
init_std=$(yq -r '.init_std' ${config_yaml})
...
data_path="${json_dir}/arxiv_text_document"   ###############　修正
...
deepspeed --master_port 29321  ${megatron_deepspeed_dir}/pretrain_gpt.py \
    ${megatron_options} \
    ${data_options} \
    ${deepspeed_options} \
